...
{{-- FILTER BALK --}}
    <div class="mb-12 bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4">
        ...
        <div class="flex-1 relative">
            ...
            {{-- debounce.300ms zorgt dat hij pas zoekt 0.3 sec nadat je stopt met typen, voelt veel vloeiender --}}
        ...
        {{-- 2. Onderwerp / Tag Filter --}}
        <div class="w-full md:w-64">
            <div class="relative">
                <select wire:model.live="selectedTag"
                        class="w-full py-3 pl-4 pr-10 rounded-xl border-gray-200 focus:border-[#2596be] focus:ring-[#2596be] shadow-sm text-gray-700 font-medium appearance-none cursor-pointer">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        @forelse($blogs as $blog)
            <article class="flex flex-col overflow-hidden rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 bg-white border border-gray-100">
                {{-- Foto --}}
                <a href="{{ route('public.blog.show', $blog->id) }}" class="flex-shrink-0">
                    @if($blog->image_path)
                        <img class="h-56 w-full object-cover" src="{{ asset('storage/' . $blog->image_path) }}" alt="{{ $blog->title }}">
                    @else
                        <div class="h-56 w-full bg-gray-200 flex items-center justify-center text-gray-400 font-bold italic">YourAirTravel</div>
                    @endif
                </a>
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap gap-2 mb-3">
                            @foreach(array_slice($blog->tags ?? [], 0, 3) as $tag)
                                <span class="inline-flex items-center rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-bold text-blue-600">
                                    {{ $tag }}
                                </span>
                            @endforeach
                        </div>
                        ...
                        <a href="{{ route('public.blog.show', $blog->id) }}" class="mt-2 block group">
                            <p class="text-xl font-extrabold text-gray-900 group-hover:text-[#2596be] transition-colors leading-tight">
                                {{ $blog->title }}
                            </p>
                            <p class="mt-3 text-sm text-gray-500 line-clamp-3 font-medium">
                                {{ Str::limit(strip_tags($blog->content), 120) }}
                            </p>
                        </a>
                    </div>

                    {{-- Alleen Datum (Geen auteur) --}}
                    <div class="mt-6 pt-4 border-t border-gray-50">
                        <div class="flex items-center text-sm text-gray-500 font-medium">
                            {{-- Klein kalender icoontje --}}
        ...
        @empty
            <div class="col-span-full text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <p class="text-gray-500">Onze redactie is druk bezig met het schrijven van nieuwe reistips!</p>
            </div>
        @endforelse
    </div>
