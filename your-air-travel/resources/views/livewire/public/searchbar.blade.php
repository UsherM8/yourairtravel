<div class="bg-white rounded-2xl shadow-2xl p-4 md:p-6 w-full max-w-6xl mx-auto border border-gray-100">
    <form wire:submit.prevent="search" class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div x-data="{ open: false }" class="relative z-50">
                ...
                <div @click="open = !open" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-white shadow-sm text-gray-700 font-medium cursor-pointer flex justify-between items-center h-full hover:border-[#2596be]/50 transition-colors">
                    <span class="truncate">
                        {{-- DE FIX: We gebruiken '?? []' om de count() error te voorkomen --}}
                ...
                <div x-show="open" @click.outside="open = false" style="display: none;" class="absolute w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-80 overflow-y-auto">
                    <div class="p-2">
                        ...
                        @foreach($beschikbareBestemmingen as $land => $steden)
                            ...
                            {{-- DE FIX: '?? []' toegevoegd bij in_array --}}
            ...
            {{-- 2. Reisperiode --}}
            <div class="relative z-40" wire:ignore>
                ...
                <input
                    ...
                    "
                    class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-white shadow-sm text-gray-700 font-medium cursor-pointer hover:border-[#2596be]/50 transition-colors focus:ring-0 focus:outline-none"
                >
            </div>
            ...
            <div x-data="{ open: false }" class="relative z-40">
                ...
                <div @click="open = !open" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-white shadow-sm text-gray-700 font-medium cursor-pointer flex justify-between items-center h-full hover:border-[#2596be]/50 transition-colors">
                    {{-- DE FIX: In javascript || [] toegevoegd voor de zekerheid --}}
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-100 pt-4 mt-2">
            ...
            {{-- 5. Budget (DE DUBBELE RANGE SLIDER) --}}
            <div class="flex items-center space-x-2">
                ...
                <div x-data="{ open: false }" class="relative w-full z-30">
                    ...
                    <div x-show="open" @click.outside="open = false" style="display: none;" class="absolute w-full sm:w-80 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl p-5 z-50">

                        {{-- Slider 1: Minimaal Budget --}}
                        <div class="mb-6">
                            <div class="flex justify-between text-xs text-gray-500 font-bold mb-2">
                                ...
                                <span class="text-[#2596be] text-base font-black border border-gray-100 bg-gray-50 px-2 py-0.5 rounded">€{{ $min_budget }}</span>
                            </div>
                            <input
                                type="range"
                                wire:model.live.debounce.300ms="min_budget"
                                min="0" max="2000" step="50"
                                class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#2596be]"
                            >
                        </div>

                        {{-- Slider 2: Maximaal Budget --}}
                        <div>
                            <div class="flex justify-between text-xs text-gray-500 font-bold mb-2">
                                <span class="uppercase tracking-wider">Maximale Prijs</span>
                                <span class="text-[#2596be] text-base font-black border border-gray-100 bg-gray-50 px-2 py-0.5 rounded">
                                    {{ $max_budget >= 2000 ? '2000+' : '€' . $max_budget }}
                                </span>
                            </div>
                            <input
                                type="range"
                                wire:model.live.debounce.300ms="max_budget"
                                min="0" max="2000" step="50"
                                class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#e5764b]"
                            >
                        </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <div x-data="{ open: false }" class="relative w-full z-30">
                    <div @click="open = !open" class="w-full py-2 px-3 border border-gray-200 rounded-lg text-sm text-gray-600 bg-white cursor-pointer flex justify-between items-center hover:border-[#2596be]/50 transition-colors">
                        {{-- DE FIX: JS Veilige fallback toegevoegd --}}
                    ...
                    <div x-show="open" @click.outside="open = false" style="display: none;" class="absolute w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl">
                        <div class="p-1">
                            @foreach(['zon' => 'Zonvakantie ☀️', 'stad' => 'Stedentrip 🏛️', 'natuur' => 'Natuur & Actief 🌲', 'ver' => 'Verre Reizen 🌍', 'lastminute' => 'Last-Minute ⚡'] as $val => $label)
            ...
            <div class="flex items-center space-x-2">
                ...
                <div x-data="{ open: false }" class="relative w-full z-30">
                    ...
                    {{-- max-h-60 en overflow-y-auto zorgen voor een scrollbar, want de lijst is nu 15 items lang! --}}
                    <div x-show="open" @click.outside="open = false" style="display: none;" class="absolute w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                        <div class="p-1">
                            @for ($i = 1; $i <= 14; $i++)
                                <label class="flex items-center px-3 py-2 hover:bg-[#2596be]/10 rounded cursor-pointer transition-colors">
                            ...
                            <label class="flex items-center px-3 py-2 hover:bg-[#2596be]/10 rounded cursor-pointer transition-colors border-t border-gray-100 mt-1 pt-2">
                                ...
                                <span class="ml-3 text-sm font-bold text-[#2596be]">Meer dan 14 dagen</span>
                            </label>
                        </div>
                    </div>
                </div>
